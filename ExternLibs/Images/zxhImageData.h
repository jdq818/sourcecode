...
#ifndef PixelTypeDefault
#define PixelTypeDefault short
#endif
#ifndef ZXHPixelTypeDefault
#define ZXHPixelTypeDefault short
#endif
...
template <class PixelType=ZXHPixelTypeDefault>
class zxhImageDataT
{
public:
	...
	///
	virtual inline PixelType GetPixelGreyscale(int iVector[ImageDimensionMax]) const ;
	...
	virtual inline void ImageGridToWorldCoordinate(float fv[ImageDimensionMax]) const
	{ 	ImageToWorld(fv) ; 	};
	/// change world coordinate in to image grid
	virtual inline void WorldCoordinateToImageGrid(float fv[ImageDimensionMax]) const
	{ 	WorldToImage(fv) ; 	};

	/// change image grid in to world coordinate
	virtual inline void ImageToWorld(float fv[ImageDimensionMax]) const { m_ImageInfo.ImageToWorld( fv ); } ;  
	...
	;
	/// change world coordinate in to image grid
	virtual inline void WorldToImage(float fv[ImageDimensionMax]) const		{ m_ImageInfo.WorldToImage( fv ) ; } ;  
	...
	{ // fx==0 can be stored as +-1e-1000 or 0, while 1 can only be store as 1 and 1+1e-1000 =  1
	  // however, change to below causes problem in zxhtransform -addroi
	  /* 
		if( fx<=-ZXH_FloatPrecision||fx-ZXH_FloatPrecision>(m_ImageInfo.Size[0])-1||
			fy<=-ZXH_FloatPrecision||fy-ZXH_FloatPrecision>(m_ImageInfo.Size[1])-1||
			fz<=-ZXH_FloatPrecision||fz-ZXH_FloatPrecision>(m_ImageInfo.Size[2])-1||
			ft<=-ZXH_FloatPrecision||ft-ZXH_FloatPrecision>(m_ImageInfo.Size[3])-1)*/
	...
	{ // fx==0 can be stored as +-1e-1000 or 0, while 1 can only be store as 1 and 1+1e-1000 =  1
	  ...
	  /* 
		...
		for(int idim=0;idim<m_ImageInfo.Dimension;++idim)
			if(fgrid[idim]<0||fgrid[idim]>int(m_ImageInfo.Size[idim])-1)
			return false;
		return true;
	}
	...
	{
		if( f==0 ) return false ; 
		for(int id=0;id<ImageDimensionMax; ++id)
...
/*
class zxhImageDataT<short int> ____a;
class zxhImageDataT<int> ____b;
class zxhImageDataT<float> ____c;
class zxhImageDataT<char> ____d;
class zxhImageDataT<double> ____e;
class zxhImageDataT<unsigned short> ____f;
class zxhImageDataT<unsigned int> ____g;
class zxhImageDataT<unsigned char> ____h;
*/
////////////////////////////////////////////////// for zxhImageDataT class ///////////////////////////////////////
template<class PixelType>
zxhImageDataT<PixelType>::zxhImageDataT()
{
	this->m_ImageInfo.DataType=6400;
...
{
	if( size[0]*size[1]*size[2]*size[3] != m_ImageInfo.Size[0]*m_ImageInfo.Size[1]*m_ImageInfo.Size[2]*m_ImageInfo.Size[3] ||
			size[0]*size[1]*size[2]*size[3] ==1 )
	{
		if( m_pImageData != 0 )
			delete []m_pImageData ;
		m_pImageData	= new PixelType[size[0]*size[1]*size[2]*size[3]];
	}
	for(int i=0;i<ImageDimensionMax;i++)
	{
...
template<class PixelType>
inline PixelType zxhImageDataT<PixelType>::GetPixelGreyscale(int iVector[ImageDimensionMax])const
{
...
bool zxhImageDataT<PixelType>::ComputeGreyCentre(bool forcecompute)
{
	...
	zxhlfloat sumofgreyxyz[ImageDimensionMax]={0,0,0,0};
	int ico[ImageDimensionMax]={0,0,0,0};
...
typedef zxhImageDataT<ZXHPixelTypeDefault> zxhImageData;
