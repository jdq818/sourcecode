...
bool miiMinPath::FindMinPathWithintensiyLL_reb(int inum,const zxhImageInfo *pImageInfo,float nStartPoint[3],const short *sImgData ,int nMaxPath)
{
	if(inum==0)m_nEndDistmm=1;
	else
		m_nEndDistmm=0.1;
	if (m_dU == NULL || m_nFmItr == 0)
	{
...
int miiMinPath::ModifiedFindImgMidPoint_OnlyVV(miiCNode<double, int> iMinNode,bool *yFind,float CurMVector[4],const zxhImageInfo *pImageInfo,float nStartPoint[3],float nVesselEndPoint[3],const short *sImgData ,float fCurMinNodevector[3],int nMaxPath)
{


	// calculate the distance(in mm level)Add by JDQ
	miiCNode<double, float>dfMinNodeWorld=ImageTransToWorldPoint(iMinNode);
miiCNode<double, float>dfVesselWorld;
float COrgVDistmm2=CalcDistmm2(m_iOrgStartPointWorld,dfMinNodeWorld);//Original Vessel Point.

dfVesselWorld.x=nVesselEndPoint[0];
dfVesselWorld.y=nVesselEndPoint[1];
dfVesselWorld.z=nVesselEndPoint[2];
float fDistmm2=CalcDistmm2(dfMinNodeWorld,dfVesselWorld);
	miiCNode<double, int>mst=WorldTransToImagePoint(m_iStartPointWorld);;
	float CSVDistmm2=CalcDistmm2(dfMinNodeWorld,m_iStartPointWorld);
	float SLVVector[3]={0};//from last startpoint to current startpoint 
	float SNVVector[3]={0};//Current Segpoint  to startpoint on the vessel(approximate)
	float SNVVectorU1[3]={0};//Current Segpoint  to startpoint on the vessel from u1
	...
	SNVVectorU1[1]=fCurMinNodevector[1];
	SNVVectorU1[2]=fCurMinNodevector[2];
	float costhetaU1=zxh::VectorOP_Cosine( SNVVectorU1,SMVector, 3 );
	//Get the tangent of point D and calculate the distance coming from current point

	float fconstcosine=0.8660;
	float fconstcosineU1=0.8660;
	float fminseglenth=1;
	float fseglenth=3;
	float fmaxselengt=6;
	float fbei=0.9;
	if (fDistmm2< fminseglenth*fminseglenth) //theta<30бу
		{
	...
	if (m_nFMMEvlNum == 1)
	{
		...
		//m_fModelVecWorld[1]=m_vModelPoints[1].y-m_vModelPoints[0].x;
		//m_fModelVecWorld[2]=m_vModelPoints[1].z-m_vModelPoints[0].x;
		if (CSVDistmm2 > fseglenth*fseglenth&&CSVDistmm2 < fmaxselengt*fmaxselengt)//
		{
			//first tangential direction have already initialized
			m_iEndPointWorld.x = dfMinNodeWorld.x;
			m_iEndPointWorld.y =dfMinNodeWorld.y;
	...
	else  if (m_nFMMEvlNum<31)
	{ 
		float fdislim=m_nFMMEvlNum*fseglenth;
		float fsumdist=sqrt(COrgVDistmm2);
		if (fsumdist>=fdislim&&CSVDistmm2 > fseglenth*fseglenth&&CSVDistmm2 < fmaxselengt*fmaxselengt) //theta<30бу
		{
	...
	{ 
		...
		m_iSgmtPoint=iMinNode;
		float fdislim=0.5*m_nFMMEvlNum*fseglenth+0.5*m_fBackTrackDistmmSkipSomePonts;
		if ( CSVDistmm2 >fseglenth*fseglenth&&CSVDistmm2 < fmaxselengt*fmaxselengt&&zxh::absf(costhetaU1)>fconstcosineU1) //theta<30бу to determine whether is a segment point
		{  
			// is sgm point candidate, if yes return, else assign orignal values back to m_iEndPointWorld and m_iSgmtPointWorld
			if(!IsSegmentBPoint(m_iSgmtPointWorld,pImageInfo,nStartPoint,sImgData ,nMaxPath))
				*yFind=true;
		...
		else if( CSVDistmm2 > fseglenth*fseglenth&&CSVDistmm2 < fmaxselengt*fmaxselengt)

		{
			//fconstcosine=zxh::maxf(0.75*fconstcosine,0);
			//fconstcosineU1=zxh::maxf(0.75*fconstcosineU1,0.5);
			if (zxh::absf(costhetaU1)>fconstcosineU1) //theta<30бу to determine whether is a segment point
			{  
				// is sgm point candidate, if yes return, else assign orignal values back to m_iEndPointWorld and m_iSgmtPointWorld
				if(!IsSegmentBPoint(m_iSgmtPointWorld,pImageInfo,nStartPoint,sImgData ,nMaxPath))
					*yFind=true;
		...
		else if(CSVDistmm2 > fseglenth*fseglenth &&CSVDistmm2 < fmaxselengt*fmaxselengt) // to determine whether is a bad point 
		{  
			// NOT segment point, (1) check whether bad point; (2) assign orginal values back to m_iEndPointWorld and m_iSgmtPointWorld
			if( IsSegmentBPoint(m_iSgmtPointWorld,pImageInfo,nStartPoint,sImgData ,nMaxPath))
			{
...
void miiMinPath::CheckIsSgmtPointAndBadPoint_OnlyVV(miiCNode<double, int>iMinNode,const zxhImageInfo *pImageInfo,float nStartPoint[3],float nVesselEndPoint[3],const short *sImgData,int nMaxPath,float fCurMinNodevector[3],float fmv[3],bool &blSegPointFind)
{
	...
	{ 
		CurMVector[3]=CalcDistmm2(m_vModelPointsWorld[0],m_vModelPointsWorld[1]);
		m_nFmItr=ModifiedFindImgMidPoint_OnlyVV(iMinNode,&blSegPointFind,CurMVector,pImageInfo,nStartPoint,nVesselEndPoint,sImgData ,fCurMinNodevector,nMaxPath);
