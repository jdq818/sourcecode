...
public:
	...
	///miiMinPathModel(int nImgWX, int nImgWY, int nImgWZ, const zxhImageInfo *pBaseImgInfo, \
		int nMaxSgmtDist, bool bReadMeanFlg = false);
	///Change by JDQ
	...
	void ModifiedFastMarchingInitLL(const short *sImgData, const zxhImageInfo *pImgInfo, \
		...
		void ModifiedFastMarchingInit_OnlyVV(const short *sImgData, const zxhImageInfo *pImgInfo, \
		const short *sVslsData, const zxhImageInfo *pVslsInfo, \
		float nStartPoint[], float nEndPoint[], int nMethod = 4);
	...
	int DFM_revo_rebu_OnlyVV(int nMaxItrNum,int i,const short *sImgData,const zxhImageInfo *pImageInfo,float nStartPoint[3],float nVesselEndPoint[3],int nMaxPath,const zxhImageDataT<short> &imgReadLineMask,vector< miiCNode<double, float> > &vUVectorPoint);//copy ModifiedFastMarchingEvolutiondontMoveModelwithFindMinPathPointCNewLL_DeaFM_DN_VVSel_VSPSC_BPD; but change for rebuttal media vessel vctor only
...
private:
	...
	bool UpdateModelSgmtPointNew_OnlyVV();//limit the max length between point C and point D
	...
	float CalcTheta_OnlyVV(float v1[3],float m1[3]);
