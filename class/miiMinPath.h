...
// 		float nStartPoint[], float nEndPoint[], int nMethod) = 0;
	...
	///
	bool FindMinPathWithintensiyLL_reb(int inum,const zxhImageInfo *pImageInfo,float nStartPoint[3], const short *sImgData,int nMaxPath = 2000);//for rebuttal
	...
	int ModifiedFindImgMidPoint_OnlyVV(miiCNode<double, int> iMinNode,bool *blSegPointFind,float SLNVector[4],const zxhImageInfo *pImageInfo,float nStartPoint[3],float nVesselEndPoint[3],const short *sImgData ,float fCurMinNodevector[3],int nMaxPath);//copy from ModifiedFindImgMidPoint_DistanceThresholdPlusVectorAngledontMoveSetPointBMLL_JDQN_VSPSC_BPD
	...
	void CheckIsSgmtPointAndBadPoint_OnlyVV(miiCNode<double, int>iMinNode,const zxhImageInfo *pImageInfo,float nStartPoint[3],float nVesselEndPoint[3],const short *sImgData,int nMaxPath,float fCurMinNodevector[3],float fmv[3],bool &blSegPointFind);
...
public:
	...
	zxhImageData m_zxhNBUvalueImg;//short *sBadPointMask;	//to set u value as an image;
	zxhImageData m_zxhNBPvalueImg;//short *sBadPointMask;	//to set badpoints'neighbour points which is far as mask;
	zxhImageData m_zxhNBSvalueImg;//dimS value'sreciprocal
	zxhImageData m_zxhNBDvalueImg;//dimD value.reciprocal
	zxhImageData m_zxhNBD_SegvalueImg;//dimD in selecting segmentpoint value.reciprocal
