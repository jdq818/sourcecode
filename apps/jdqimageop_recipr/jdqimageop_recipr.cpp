...
int main(int argc, char *argv[])
{
	...
	{
		cerr << "Usage: " << endl;
		cerr << "jdqimageop_recipr	imageRaw(.nii)	 Resultjdqimageop_recipr£¨nii£©" << endl;
		return -1;
	}
	string strFileNameRaw =string(argv[1]);//"F:/Coronary_0/Coronary_Niessen/CoronaryMasks/DataMaskOutLung/mol_image00.nii";  
	char  *ResultimgName =argv[2];//"F:/Coronary_0/Coronary_Niessen/ProcessByLL/training/dataset00/rightostium_image.txt";

	/*string strFileNameRaw ="F:/Coronary_0/Coronary_Niessen/ProcessByLL/training/dataset00/medial.nii.gz";  
	string ResultimgName ="F:/Coronary_0/Coronary_Niessen/ProcessByLL/training/dataset00/proc_medial.nii.gz";*/
	zxhImageDataF imgReadRaws,imgResultRaws;//
	...
	imgResultRaws.NewImage(imgReadRaws.GetImageInfo());
	int nImWX, nImWY, nImWZ, nImWT;
	imgResultRaws.GetImageSize(nImWX, nImWY, nImWZ, nImWT);
	//output nostium into resultfile
	//for (int i = 0; i < nImWX * nImWY * nImWZ; i++)
	//{
	//	float y=imgReadRaws.GetPixelGreyscale(i);
	//	float x=1.0/(imgReadRaws.GetPixelGreyscale(i)+0.000001);
	//	imgResultRaws.SetImageData(i,x);
	//}	
	for(int nx=0;nx<nImWX;nx++)
		for(int ny=0;ny<nImWY;ny++)
			for(int nz=0;nz<nImWZ;nz++)
			{
				if(nx==57&&ny==40&&nz==277)
				{
				int m=0;
				}
				float y=imgReadRaws.GetPixelGreyscale(nx,ny,nz,0);
				float x=1.0/(y+0.001);
				imgResultRaws.SetPixelByGreyscale(nx,ny,nz,0,x);//sAMinNodeImg[i]=m_iFarPValue;
				

			}
	zxh::SaveImage(&imgResultRaws,ResultimgName);
	cout << "Reciprocal Image has been generated!" << endl;
