...
bool ReadPointTxt(char *filename,vector<PointCordTypeDef> &cl)
{
	...
	strctTempPoint.x =-1*atof(strNum.c_str());
	//fMean=1024*fMean/1896;//Add by JDQ
	// read 2nd number
	nStart = nEnd + 1;
	nEnd = strLine.find_first_of(TAB_CHAR, nStart);
	strNum.assign(strLine, nStart, nEnd - nStart);
	strctTempPoint.y =-1*atof(strNum.c_str());
	// read 3rd number
	nStart = nEnd + 1;
	nEnd = strLine.find_first_of(TAB_CHAR, nStart);
	strNum.assign(strLine, nStart, nEnd - nStart);
...
bool LengthenForwardMCline(vector<miiCNode<double,float>>vPointWorld,vector<PointCordTypeDef>&vLengthenPointWorld)
{
	...
	float step=0.1;
	for(int k=0;k<extendLENGTHmm/step;k++)
	{
	XPointWorld.x=vLengthenPointWorld[0].x-k*step*tempVec[0];
	XPointWorld.y=vLengthenPointWorld[0].y-k*step*tempVec[1];
	XPointWorld.z=vLengthenPointWorld[0].z-k*step*tempVec[2];
	vLengthenPointWorld.push_back(XPointWorld);
	}
...

{
	...
	//char *MCLinFileName ="F:/Coronary_0/ZXHJDQCAEDMP/DHResDFM_VVS_VSPSC6_BPD/unseen05_resultsSMD/vessel0/SMDMCLine.vtk";
	//char *chResultPathNameVTK="F:/Coronary_0/ZXHJDQCAEDMP/DHResDFM_VVS_VSPSC6_BPD/unseen05_resultsSMD/vessel0/SMDLine_EXT.vtk";
	//char *chResultPathNameTXT="F:/Coronary_0/ZXHJDQCAEDMP/DHResDFM_VVS_VSPSC6_BPD/unseen05_resultsSMD/vessel0/SMDLine_EXT.txt";
		...
		else if(strcmp(chext,".txt")==0)// read vtk file
	{
		...
		if(DetectFile)
		{
			ReadPointTxt(MCLinFileName, vPathPointsWorld);
			//cout<<'size'<<vPathPointsWorld.size()<<endl;
		}
