		...
		for(int iz=0;iz<ImgNewRawSize[2];++iz)
			for(int iy=0;iy<ImgNewRawSize[1];++iy)
				for(int ix=0;ix<ImgNewRawSize[0];++ix)
				{
					...
					double LOCALMIN=ZXH_InfiniteLargeFloat;double dNewImgVal=0;double dNewVslVal=0;double dVsVal=0;
						...
						for(locali[1]=itoImg[1]-SearchiRange[1];locali[1]<=itoImg[1]+SearchiRange[1];locali[1]++)
							for(locali[0]=itoImg[0]-SearchiRange[0];locali[0]<=itoImg[0]+SearchiRange[0];locali[0]++)
							{
								...
								if ( dImgVal < m_fMean)
								{
									...
									dSims= 1;
								dvmultiplys=1/((m_sNormVes[locali[2] * nImgWY * nImgWX + locali[1] * nImgWX + locali[0]])*dSims+0.00000001);
								float s=dSims;
								float v=m_sNormVes[locali[2] * nImgWY * nImgWX + locali[1] * nImgWX + locali[0]];
								if (dvmultiplys<LOCALMIN)
								{
									...
									dVsVal=m_sVes[locali[2] * nImgWY * nImgWX + locali[1] * nImgWX + locali[0]];
									
									LOCALMIN=dvmultiplys;
								}

							}
							imgReadNewRaw->SetImageData(iz * ImgNewRawSize[1] * ImgNewRawSize[0] + iy* ImgNewRawSize[0] +ix,dNewImgVal);
							imgReadNewVsls->SetImageData(iz * ImgNewRawSize[1] * ImgNewRawSize[0] + iy* ImgNewRawSize[0] +ix,dNewVslVal);
							//imgReadNewRaw->SetPixelByGreyscale(ix,iy,iz,0,dNewImgVal);//Normalized intensity
							//imgReadNewVsls->SetPixelByGreyscale(ix,iy,iz,0,dNewVslVal);//Normalized intensity
...
bool ResoResaTraverseIntensityN(const  short *sImgData, const  short *sWinImgData,const zxhImageInfo *pImgInfo,float m_fMean,float m_fStdDev,const short *sVslsData, const short *sWinVslsData,const zxhImageInfo *pVslsInfo,int SearchiRange[4],zxhImageDataT<short> *imgReadNewRaw,zxhImageDataT<short> *imgReadNewVsls,const short *sVslsDataX,zxhImageDataT<short> *imgReadNewVslsX,const short *sVslsDataY,zxhImageDataT<short> *imgReadNewVslsY,const short *sVslsDataZ,zxhImageDataT<short> *imgReadNewVslsZ)
{       int nImgWX=pVslsInfo->Size[0];
		int nImgWY=pVslsInfo->Size[1];
		int nImgWZ=pVslsInfo->Size[2];
	double *m_sVes=new double[nImgWX * nImgWY * nImgWZ] ;//add by JDQ
		...
		for(int iz=0;iz<ImgNewRawSize[2];++iz)
			for(int iy=0;iy<ImgNewRawSize[1];++iy)
				for(int ix=0;ix<ImgNewRawSize[0];++ix)
				{
					...
					int locali[4]={0};
					int newix,newiy,newiz,newit;
					double LOCALMIN=ZXH_InfiniteLargeFloat;double dNewImgVal=0;double dNewVslVal=0;double dVsVal=0;double dNewVslsXVal=0;double dNewVslsYVal=0;double dNewVslsZVal=0;	
					for(locali[2]=itoImg[2]-SearchiRange[2];locali[2]<=itoImg[2]+SearchiRange[2];locali[2]++)
						for(locali[1]=itoImg[1]-SearchiRange[1];locali[1]<=itoImg[1]+SearchiRange[1];locali[1]++)
							for(locali[0]=itoImg[0]-SearchiRange[0];locali[0]<=itoImg[0]+SearchiRange[0];locali[0]++)
							{
								...
								if (dvmultiplys<LOCALMIN)
								{
									...
									dNewVslVal=sVslsData[newiz * nImgWY * nImgWX + newiy* nImgWX +newix];
									dVsVal=m_sVes[locali[2] * nImgWY * nImgWX + locali[1] * nImgWX + locali[0]];
									dNewVslsXVal=sVslsDataX[newiz * nImgWY * nImgWX + newiy* nImgWX +newix];
									dNewVslsYVal=sVslsDataY[newiz * nImgWY * nImgWX + newiy* nImgWX +newix];
									dNewVslsZVal=sVslsDataZ[newiz * nImgWY * nImgWX + newiy* nImgWX +newix];
									LOCALMIN=dvmultiplys;
								}
							...
							imgReadNewRaw->SetImageData(iz * ImgNewRawSize[1] * ImgNewRawSize[0] + iy* ImgNewRawSize[0] +ix,dNewImgVal);
							imgReadNewVsls->SetImageData(iz * ImgNewRawSize[1] * ImgNewRawSize[0] + iy* ImgNewRawSize[0] +ix,dNewVslVal);
						    imgReadNewVslsX->SetImageData(iz * ImgNewRawSize[1] * ImgNewRawSize[0] + iy* ImgNewRawSize[0] +ix,dNewVslsXVal);
							imgReadNewVslsY->SetImageData(iz * ImgNewRawSize[1] * ImgNewRawSize[0] + iy* ImgNewRawSize[0] +ix,dNewVslsYVal);
							imgReadNewVslsZ->SetImageData(iz * ImgNewRawSize[1] * ImgNewRawSize[0] + iy* ImgNewRawSize[0] +ix,dNewVslsZVal);
							//imgReadNewRaw->SetPixelByGreyscale(ix,iy,iz,0,dNewImgVal);//Normalized intensity
							//imgReadNewVsls->SetPixelByGreyscale(ix,iy,iz,0,dNewVslVal);//Normalized intensity
...
bool CalcUnseenMeanStdNew(double gAortaMean[2],double gAortaStdDev[2],double &fUnseenCAMean,double &fUnseenCAStdDev)
{
	fUnseenCAMean=gAortaMean[1];//fUnseenCAMean=fAtlsCAMean-gAortaMean[0]+gAortaMean[1];
	fUnseenCAStdDev=gAortaStdDev[1];
	return true;
...
int main(int argc, char *argv[])//without intensity nomalization
{
	...
	//{
	//cerr << "usage: " << endl;
	//cerr << "zxhcaedmpresoimage	modelimage-number" << endl;
	//return -1;
	//} 
	//char *nself=argv[1];
	//int k=atoi(nself);
	...
	//if (k<10)
	//{
	//	strunseenNUM="0"+string(nself);
	//}
	//else
	//	strunseenNUM=string("10");//more than 10;

	//string strFileNameVsls = "F:/Coronary_0/Coronary_Niessen/CoronaryMasks/DataMaskOut_ZXH/moeb2_image"+strunseenNUM+"_vesselness.nii";
	//string strFileNameRaw = "F:/Coronary_0/Coronary_Niessen/CoronaryMasks/DataMaskOutLung/mol_image"+strunseenNUM+".nii";	
	//
	//string strResultPath;
	//char *chResultPath =new char[1024];
	//string strAtlsMeanStdDevFileName;
	//char *chAtlsMeanStdDevFileName=new char[1024];
	//string strUnseenMeanStdDevFileName;
	//char *chUnseenMeanStdDevFileName=new char[1024];
	//string strMolPointsIntensityFileName;
	...
	//		if(i!=k)
	//		{
	//			stringstream ss2,sstr;  
	//			string strModeNUM;  
	//			ss2 << i;  
	//			ss2 >> strModeNUM;  
	//			if (i<10)
	//				strModeNUM="0"+strModeNUM;
	//			strResultPath = "F:/Coronary_0/Coronary_Niessen/ProcessByJDQ/training/dataset"+strModeNUM+"/vessel"+strVeslNUM;
	//			sstr.clear();
	//			sstr<<strResultPath;
	//			sstr>>chResultPath;
	//			strAtlsMeanStdDevFileName="F:/Coronary_0/meanstd/atlas"+strModeNUM+"_mean_std.txt";
	//			sstr.clear();
	//			sstr<<strAtlsMeanStdDevFileName;
	//			sstr>>chAtlsMeanStdDevFileName;
	//			strUnseenMeanStdDevFileName="F:/Coronary_0/meanstd/atlas"+strModeNUM+"_to_atlas"+strunseenNUM+"_mean_std.txt";
	//			sstr.clear();
	//			sstr<<strUnseenMeanStdDevFileName;
	//			sstr>>chUnseenMeanStdDevFileName;
	//			strMolPointsIntensityFileName="F:/Coronary_0/Coronary_Niessen/mean_centerline_intensity/image"+strModeNUM+"_"+strModeNUM+"mline_v"+strVeslNUM+"_inten.txt";
	//			sstr.clear();
	...
	{
		cerr << "usage: " << endl;
		cerr << "zxhcaedmpresoimage	vessleness.nii raw.nii rawresult vslsresult atlamean unseenmean modepointintensity" << endl;
		return -1;
	} 
	//string strFileNameVsls = string(argv[1]);
	//string strFileNameRaw = string(argv[2]);
	//char *chResoRawName = argv[3];
	//char *chResoVslsName = argv[4];
	//char *chAtlsMeanStdDevFileName = argv[5];
	//char *chUnseenMeanStdDevFileName = argv[6];
	//char *chMolPointsIntensityFileName=argv[7];
	string strFileNameVsls =string(argv[1]);
	string strFileNameRaw =string(argv[2]);
	string strFileNameVslsX =string(argv[3]);
	string strFileNameVslsY =string(argv[4]);
	string strFileNameVslsZ =string(argv[5]);
	char *chResoRawName =argv[6];
	char *chResoVslsName =argv[7];
	char *chResoVslsNameX =argv[8];
	char *chResoVslsNameY =argv[9];
	char *chResoVslsNameZ =argv[10];
	char *chUnseenMeanStdDevFileName=argv[11];

	/*string strFileNameVsls ="F:/Coronary_0/trainningdataZXHCAEDMP/Occlusion/moeb_image04_vesselness_refgau4.nii.gz";
	string strFileNameRaw ="F:/Coronary_0/trainningdataZXHCAEDMP/Occlusion/mol_image04_refgau4.nii.gz";
	char *chResoRawName ="F:/Coronary_0/Coronary_Niessen/ProcessByJDQ/training/dataset00/Resmol_image04_refgau4.nii.gz";
	char *chResoVslsName ="F:/Coronary_0/Coronary_Niessen/ProcessByJDQ/training/dataset00/Remoeb_image04_vesselness_refgau4.nii.gz";
	char *chUnseenMeanStdDevFileName="F:/Coronary_0/meanstd/atlas04_mean_std.txt";*/

	//string strFileNameVsls ="F:/Coronary_0/Coronary_Niessen/CoronaryMasks/DataMaskOutLung/moeb_image00_vesselness.nii.gz";
	//string strFileNameRaw ="F:/Coronary_0/Coronary_Niessen/CoronaryMasks/DataMaskOutLung/mol_image00.nii.gz";
	...
	//char *chResoRawName ="F:/Coronary_0/Coronary_Niessen/ProcessByJDQ/training/dataset00/Resmol_image00_new.nii.gz";
	//char *chResoVslsName ="F:/Coronary_0/Coronary_Niessen/ProcessByJDQ/training/dataset00/Remoeb_image00_vesselness_new.nii.gz";
	//char *chResoVslsNameX ="F:/Coronary_0/Coronary_Niessen/ProcessByJDQ/training/dataset00/Res_image00_vesselness_new_X.nii.gz";
	//char *chResoVslsNameY ="F:/Coronary_0/Coronary_Niessen/ProcessByJDQ/training/dataset00/Res_image00_vesselness_new_Y.nii.gz";
	//char *chResoVslsNameZ ="F:/Coronary_0/Coronary_Niessen/ProcessByJDQ/training/dataset00/Res_image00_vesselness_new_Z.nii.gz";
	...
	//read model points' intensity from txt;
	//vector<double>vModelIntensity;
	//ReadModelPoiIntenFromTxt(chMolPointsIntensityFileName,vModelIntensity);
	//float inten=0;
	//for(int i=0;i<vModelIntensity.size();i++)
	//{
	//	inten=inten+vModelIntensity[i];

	//}

	//fAtlsCAMean=inten/(vModelIntensity.size());

	//CalcUnseenMeanStd(gAortaMean,gAortaStdDev,fAtlsCAMean,fUnseenCAMean,fUnseenCAStdDev);
	CalcUnseenMeanStdNew(gAortaMean,gAortaStdDev,fUnseenCAMean,fUnseenCAStdDev);
	// for Model with the correction

	bool bRunResult;
	bool resave=false,resample=true;
	//read a "nifti" file (imgdata and vesselness data)
	zxhImageDataT<short> imgReadVsls, imgReadRaw, imgReadVslsX,imgReadVslsY,imgReadVslsZ;//Change by JDQ
	zxhImageDataT<short> *imgReadNewRaw=new zxhImageDataT<short>;
	zxhImageDataT<short> *imgReadNewVsls=new zxhImageDataT<short>;
	zxhImageDataT<short> *imgReadNewVslsX=new zxhImageDataT<short>;
	zxhImageDataT<short> *imgReadNewVslsY=new zxhImageDataT<short>;
	zxhImageDataT<short> *imgReadNewVslsZ=new zxhImageDataT<short>;
	if( zxh::OpenImage( &imgReadRaw, strFileNameRaw ) == false )
	{
		std::cerr << "Raw image(nifti-file) is not found!"; 
		return -1;
	...
	*/
	...
	imgReadNewRaw->NewImage(imgReadRaw.GetDimension(),size,fNewImgSpacing, imgReadRaw.GetImageInfo());
	imgReadNewVsls->NewImage(imgReadVsls.GetDimension(),size,fNewImgSpacing, imgReadVsls.GetImageInfo());
	imgReadNewVslsX->NewImage(imgReadVsls.GetDimension(),size,fNewImgSpacing, imgReadVsls.GetImageInfo());
	imgReadNewVslsY->NewImage(imgReadVsls.GetDimension(),size,fNewImgSpacing, imgReadVsls.GetImageInfo());
	imgReadNewVslsZ->NewImage(imgReadVsls.GetDimension(),size,fNewImgSpacing, imgReadVsls.GetImageInfo());
	// get the vesselness data 
	const short *sVslsData = imgReadVsls.GetImageData();	
	//get the image size
	int nImWX, nImWY, nImWZ, nImWT;
	...
	//get u1 vector
	const short *sVslsDataX = imgReadVslsX.GetImageData();
	const short *sVslsDataY = imgReadVslsY.GetImageData();
	const short *sVslsDataZ = imgReadVslsZ.GetImageData();
	...
	ResoResaTraverseIntensityN(sImData,sWinImg, imgReadRaw.GetImageInfo(),fUnseenCAMean, fUnseenCAStdDev, \
		sVslsData,sWinVsls, imgReadVsls.GetImageInfo(),SearchRange,imgReadNewRaw,imgReadNewVsls,sVslsDataX,imgReadNewVslsX,sVslsDataY,imgReadNewVslsY,sVslsDataZ,imgReadNewVslsZ);
	...
	//strcat(chFileName1, ".nii.gz");
	const zxhImageDataT<short> *img=imgReadNewRaw;
	zxh::SaveImage(img,chResoRawName);
	const zxhImageDataT<short> *vsl=imgReadNewVsls;
	zxh::SaveImage(vsl,chResoVslsName);
	const zxhImageDataT<short> *vslx=imgReadNewVslsX;
	zxh::SaveImage(vslx,chResoVslsNameX);
	const zxhImageDataT<short> *vsly=imgReadNewVslsY;
	zxh::SaveImage(vsly,chResoVslsNameY);
	const zxhImageDataT<short> *vslz=imgReadNewVslsZ;
	zxh::SaveImage(vslz,chResoVslsNameZ);
	delete[]sWinImg;
	delete[]sWinVsls;
